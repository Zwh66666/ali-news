...
# 命令行参数
parser = argparse.ArgumentParser(description='CatBoost 排序')
...
log.info(f'CatBoost 排序，mode: {mode}')
...
def train_model(df_feature, df_query):
    ...
    for fold_id, (trn_idx, val_idx) in enumerate(
        ...
        pred_val = cat_model.predict_proba(X_val)[:, 1]
        df_oof = df_train.iloc[val_idx][['user_id', 'article_id', ycol]].copy()
        df_oof['pred'] = pred_val
        oof.append(df_oof)

        pred_test = cat_model.predict_proba(df_test[feature_names])[:, 1]
        ...
        # 保存模型
        joblib.dump(cat_model, f'../user_data/model/catboost{fold_id}.pkl')
...
def online_predict(df_test):
    ...
    for fold_id in tqdm(range(5)):
        model = joblib.load(f'../user_data/model/catboost{fold_id}.pkl')
