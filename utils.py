...
def gen_sub(prediction):
    ...
    part_users = prediction['user_id'].unique()
    result = []

    for user in part_users:
        user_pred = prediction[prediction['user_id'] == user]
        user_pred = user_pred.sort_values('pred', ascending=False)
        rec_articles = user_pred['article_id'].values.tolist()
        result.append({'user_id': user, 'article_id': ' '.join(map(str, rec_articles))})
